/*
  QTRSensors.cpp - Arduino library for using Pololu QTR reflectance
	...
	* SensorInfravermelhoRC should be used for QTR-1RC and QTR-8RC sensors.
	* SensorInfravermelhoAnalog should be used for QTR-1A and QTR-8A sensors.
*/
...
void SensorInfravermelho::init(unsigned char *pins, unsigned char numSensors, 
...
// surface or a void).
void SensorInfravermelho::read(unsigned int *sensor_values, unsigned char readMode)
{
	...
	if (_type == QTR_RC)
	{
		((SensorInfravermelhoRC*)this)->readPrivate(sensor_values);
		emittersOff();
		if(readMode == QTR_EMITTERS_ON_AND_OFF)
			((SensorInfravermelhoRC*)this)->readPrivate(off_values);
	}
	else
	{
		((SensorInfravermelhoAnalog*)this)->readPrivate(sensor_values);
		emittersOff();
		if(readMode == QTR_EMITTERS_ON_AND_OFF)
			((SensorInfravermelhoAnalog*)this)->readPrivate(off_values);
	}
...
void SensorInfravermelho::emittersOff()
{
...
void SensorInfravermelho::emittersOn()
{
	if (_emitterPin == QTR_NO_EMITTER_PIN)
		return;
	pinMode(_emitterPin, OUTPUT);
...
void SensorInfravermelho::resetCalibration()
{
	unsigned char i;
	for(i=0;i<_numSensors;i++)
	{
...
void SensorInfravermelho::calibrate(unsigned char readMode)
{
...
void SensorInfravermelho::calibrateOnOrOff(unsigned int **calibratedMinimum,
...
void SensorInfravermelho::readCalibrated(unsigned int *sensor_values, unsigned char readMode)
{
	int i;
...
int SensorInfravermelho::readLine(unsigned int *sensor_values,
...
SensorInfravermelhoRC::SensorInfravermelhoRC(unsigned char* pins,
...
void SensorInfravermelhoRC::init(unsigned char* pins,
	unsigned char numSensors, unsigned int timeout, unsigned char emitterPin)
{
	SensorInfravermelho::init(pins, numSensors, emitterPin, QTR_RC);
...
void SensorInfravermelhoRC::readPrivate(unsigned int *sensor_values)
{
	unsigned char i;
...
SensorInfravermelhoAnalog::SensorInfravermelhoAnalog(unsigned char* pins,
...
// or if you just want the emitters on all the time and don't want to
// use an I/O pin to control it, use a value of 255 (QTR_NO_EMITTER_PIN).
void SensorInfravermelhoAnalog::init(unsigned char* pins,
	unsigned char numSensors, unsigned char numSamplesPerSensor,
	unsigned char emitterPin)
{	
	SensorInfravermelho::init(pins, numSensors, emitterPin, QTR_A);
...
void SensorInfravermelhoAnalog::readPrivate(unsigned int *sensor_values)
{
	unsigned char i, j;

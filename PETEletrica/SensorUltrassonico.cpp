...
SensorUltrassonico::SensorUltrassonico(int tp, int ep)
    {
    pinMode(tp, OUTPUT);
    pinMode(ep, INPUT);
    _trigPin = tp;
...
long SensorUltrassonico::timing()
    {
...
float SensorUltrassonico::convert(long microsec, int metric)
    {
    // microsec / 29 / 2;
...
void SensorUltrassonico::setDivisor(float value, int metric)
    {
...
bool SensorUltrassonico::sampleCreate(size_t numBufs, ...)
    {
    bool result = false;
    va_list ap;
    _numBufs = numBufs;
...
void SensorUltrassonico::sampleClear()
    {
    if(_pBuffers)
        {
        BufCtl *buf;
...
float SensorUltrassonico::unbiasedStdDev(float value, size_t bufNum)
    {
    float result = 0.0;
...
void SensorUltrassonico::_sampleUpdate(BufCtl *buf, float msec)
    {
...
void SensorUltrassonico::_freeBuffers()
    {
    if(_pBuffers)
        {
        BufCtl *buf;
